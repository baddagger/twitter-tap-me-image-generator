...
const translations = {
  zh: {
    ...
    "desc-resolution": "分辨率需采用推特预览尺寸的整数倍（如 1400px 或 2112px），以使插值缩放时的像素采样网格与交错图案精确对齐。",
    "label-compression": "PNG 压缩方式",
    "option-lossless": "无损",
    "option-q256": "256 色 (推荐)",
    "option-q128": "128 色",
    "option-q64": "64 色",
    "option-q32": "32 色",
    "option-q16": "16 色",
    ...
    "timeline-desc": "时间线浏览时，封面图的透明区域会触发系统 Alpha 阈值归零，从而完全隐藏底图。但不透明区域仍会产生 50% 混色叠加。若封面图上有不透明主体，底图在相同位置也必须是不透明的（即有底图元素存在），否则封面主体会因 Alpha 均值过低而一同被截断吞噬变透明。",
    "click-title": "点击大图预览",
    "click-hint": "模拟大图：以接近 1:1 的原始像素网格呈现",
    "click-desc": "用户点击或查看大图时，图像以原始分辨率或不同的缩放网格呈现，隐藏的里图内容得以清晰显露。",
    ...
    "label-force-transparent": "强制左上角首个像素完全透明",
    "desc-force-transparent": "将左上角首个像素 (1x1) 设为完全透明 (Alpha=0)。这可以欺骗推特 (Twitter) 强制保留无损 PNG 格式，防止其被优化转换为 JPEG 导致双层图失效。",
  ...
  en: {
    ...
    "desc-resolution": "Resolution must be an exact multiple of Twitter preview sizes (e.g., 1400px or 2112px) so that the downsampling grid aligns perfectly with the interlocking pattern.",
    "label-compression": "PNG Compression",
    "option-lossless": "Lossless",
    "option-q256": "256 Colors (Recommended)",
    "option-q128": "128 Colors",
    "option-q64": "64 Colors",
    "option-q32": "32 Colors",
    "option-q16": "16 Colors",
    ...
    "timeline-desc": "On the timeline, transparent cover areas trigger Alpha thresholding to fully hide the background. However, solid areas will blend 50/50. Opaque elements in your cover must be backed by opaque pixels in the hidden image, or they will be devoured/turned transparent due to low average alpha.",
    "click-title": "Click to Expand Preview",
    "click-hint": "Simulated Lightbox: Rendered with near 1:1 original pixel grid",
    "click-desc": "When expanded, the image renders at or near its original resolution, allowing the hidden image to be clearly revealed.",
    ...
    "label-force-transparent": "Force top-left pixel fully transparent",
    "desc-force-transparent": "Sets the top-left pixel (1x1) to fully transparent (Alpha=0). This forces Twitter to keep the lossless PNG format and prevents conversion to JPEG.",
  ...
  ja: {
    ...
    "desc-resolution": "解像度はTwitterのプレビューサイズの倍数（1400pxまたは2112pxなど）にする必要があり、縮小時のサンプリンググリッドとパターンを正確に整列させます。",
    "label-compression": "PNG 圧縮方式",
    "option-lossless": "ロスレス",
    "option-q256": "256色 (推奨)",
    "option-q128": "128色",
    "option-q64": "64色",
    "option-q32": "32色",
    ...
    "timeline-desc": "タイムライン閲覧時、表画像の透明領域はAlpha閾値処理によって裏画像を完全に隠しますが、不透明領域は両方の画像が混ざり合います。表画像の不透明な部分は、裏画像の同じ位置にも不透明なピクセルがないと透明化して消えてしまいます。",
    "click-title": "クリック拡大のプレビュー",
    "click-hint": "大図模擬：1:1のオリジナル画素グリッドで表示",
    "click-desc": "ユーザーがクリックして拡大すると、画像は元の解像度または異なるグリッドで描画され、裏画像の内容がクリアに表示されます。",
    ...
    "label-force-transparent": "左上の最初の1ピクセルを完全透明にする",
    "desc-force-transparent": "左上の最初の1ピクセル (1x1) を完全透明 (Alpha=0) に設定します。これにより、TwitterがロスレスPNG形式を維持し、JPEGへの変換を防ぎます。",
  ...
  ko: {
    ...
    "desc-resolution": "해상도는 트위터 프리뷰 크기의 배수(1400px 또는 2112px 등)여야 하며, 축소 시 샘플링 그리드와 패턴이 정확하게 정렬됩니다.",
    ...
    "timeline-desc": "타임라인을 탐색할 때 겉 이미지의 투명 영역은 Alpha 임계값 처리에 의해 속 이미지를 완전히 숨기지만, 불투명 영역은 두 이미지가 혼합됩니다. 겉 이미지의 불투명한 부분은 속 이미지의 동일한 위치에도 불투명한 픽셀이 있어야 투명화되어 사라지지 않습니다.",
    "click-title": "클릭 확대 미리보기",
    "click-hint": "원본 이미지 시뮬레이션: 1:1에 가까운 원본 픽셀 그리드로 표시",
    "click-desc": "사용자가 클릭하여 확대할 때 이미지 크기가 원본 크기 또는 다른 그리드로 렌더링되면서 속 이미지의 내용이 선명하게 나타납니다.",
    ...
    "label-force-transparent": "왼쪽 상단 첫 픽셀을 완전 투명으로 설정",
    "desc-force-transparent": "왼쪽 상단의 첫 픽셀 (1x1)을 완전 투명(Alpha=0)으로 만듭니다. 이렇게 하면 트위터가 무손실 PNG 형식을 강제로 유지하여 JPEG로 변환되는 것을 방지합니다.",
